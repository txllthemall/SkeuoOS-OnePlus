...
from liquid_glass_v6.material import BakeFlags, MaterialParams, bake_static_rgba

OUT = Path(__file__).resolve().parents[2] / "build" / "liquid-glass-v6"
SIZE = 640
...
def container_only():
    ...
    rgba = bake_static_rgba(maps, MaterialParams(), BakeFlags())
    return Image.fromarray(rgba, "RGBA"), maps
...
def main() -> None:
    ...
    new.save(OUT / "github_v6_rgba.png")
    material_lab(new).save(OUT / "preview_v6_master.png")
    for kind in ("midtone", "dark", "bright", "warm", "blue"):
        composite_center(wallpaper(kind, (900, 900)), new, 390).save(OUT / f"preview_v6_master_{kind}.png")

    launcher_scale_suite(new).save(OUT / "preview_v6_launcher_scale.png")
    composite_center(wallpaper("midtone", (900, 900)), nospec, 390).save(OUT / "preview_v6_no_specular.png")
    composite_center(wallpaper("midtone", (900, 900)), noshadow, 390).save(OUT / "preview_v6_no_shadow.png")
    composite_center(wallpaper("midtone", (900, 900)), norim, 390).save(OUT / "preview_v6_no_rim.png")
    composite_center(wallpaper("midtone", (900, 900)), cont, 390).save(OUT / "preview_v6_container_only.png")
    composite_center(wallpaper("midtone", (900, 900)), gly, 390).save(OUT / "preview_v6_glyph_only.png")

    ab_board(old, new).save(OUT / "preview_ANDROID_BRUTAL_AB.png")
    difference_heatmap(old, new, wallpaper("midtone", (900, 900))).save(OUT / "preview_v6_difference_heatmap.png")

    map_to_gray(maps.front_height).save(OUT / "preview_v6_height.png")
    map_to_gray(maps.back_height).save(OUT / "preview_v6_back_height.png")
    map_to_gray(maps.thickness).save(OUT / "preview_v6_thickness.png")
    normal_map_image(maps.front_normals).save(OUT / "preview_v6_normals.png")
    normal_map_image(maps.back_normals).save(OUT / "preview_v6_back_normals.png")

    render_optical_preview(maps, wallpaper("highcontrast", (SIZE, SIZE)), gain=1.18).save(OUT / "preview_v6_refraction_stress.png")
    render_optical_preview(maps, wallpaper("midtone", (SIZE, SIZE)), gain=1.05).save(OUT / "preview_v6_optical_midtone.png")

    a = np.asarray(new.convert("RGBA"), dtype=np.float32)
    gm = maps.glyph_mask > 0.5
    cm = maps.container_mask > 0.5
    edge = (maps.q > 0.70) & cm
    core = (maps.q < 0.45) & cm & (~gm)
    data = {
        "renderer": "liquid_glass_v6_signed_dual_polarity",
        "size": SIZE,
        "static_background_sampling": False,
        "container_alpha_mean": float(a[..., 3][cm].mean() / 255.0),
        "container_edge_alpha_mean": float(a[..., 3][edge].mean() / 255.0) if np.any(edge) else 0.0,
        "container_core_alpha_mean": float(a[..., 3][core].mean() / 255.0) if np.any(core) else 0.0,
    ...
    (OUT / "v6_diagnostics.json").write_text(json.dumps(data, indent=2, sort_keys=True), encoding="utf-8")
