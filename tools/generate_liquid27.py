...
def render(name,bgspec,kind):
    canvas=Image.new('RGBA',(WORK,WORK),(0,0,0,0)); canvas.alpha_composite(background(bgspec))
    for lay in glyph(kind): composite_layer(canvas,**lay)
...
def preview(images, outdir):
    show=['skeuo_phone','skeuo_messages','skeuo_camera','skeuo_photos','skeuo_settings','skeuo_mail','skeuo_maps','skeuo_clock','skeuo_weather','skeuo_notes','skeuo_calendar','skeuo_music','skeuo_appstore','skeuo_telegram','skeuo_discord','skeuo_youtube','skeuo_chrome','skeuo_spotify','skeuo_instagram','skeuo_soundcloud','skeuo_kaspi','skeuo_2gis','skeuo_chatgpt','skeuo_gamehub']
    font=ImageFont.truetype(FONT_REG,18)
    def sheet(name,bg):
        cols=6; cell=170; rows=math.ceil(len(show)/cols); W=cols*cell+60; H=rows*205+60
        ...
        for i,n in enumerate(show):
            x=30+(i%cols)*cell; y=30+(i//cols)*205; ic=images[n].resize((128,128),Image.Resampling.LANCZOS); can.paste(ic,(x+21,y),ic)
            label=n[6:].replace('_',' ')[:16]; b=d.textbbox((0,0),label,font=font); d.text((x+85-(b[2]-b[0])/2,y+142),label,font=font,fill=fg)
        can.save(outdir/name)
    sheet('preview_light.png','#eff0f3'); sheet('preview_dark.png','#17181c')

    # Full contact sheet: every generated resource.
    ...
    # Color-rich wallpaper context for transparency/readability inspection.
    ...
    for col,xy,rad in [('#7f335f',(240,280),600),('#233b8a',(850,260),650),('#693b22',(800,970),650)]:
        ...
        fullmask=Image.new('L',(W,H),0); fullmask.paste(m,(xy[0]-rad//2,xy[1]-rad//2)); wall=Image.composite(Image.new('RGB',(W,H),rgb(col)),wall,fullmask)
    ...
    # OnePlus/Home-Screen-scale check.
...
def qa(images, outdir):
    ...
    for name,(bg,kind,defaults) in ICON_SPECS.items():
        layers=glyph(kind); fg=blank_mask()
        ...
        im=images[name].convert('RGB').resize((64,64)); vals=[.2126*r+.7152*g+.0722*b for r,g,b in im.getdata()]; mean=sum(vals)/len(vals); contrast=(max(vals)-min(vals))/255
        rows.append({'icon':name,'foreground_bbox':bb,'coverage_pct':round(coverage*100,1),'center_offset_pct':[round(off[0]*100,1),round(off[1]*100,1)],'mean_luminance':round(mean,1),'contrast_estimate':round(contrast,3)})
    ...
    bad=[r for r in rows if r['coverage_pct']<6 or r['coverage_pct']>68 or abs(r['center_offset_pct'][0])>15 or abs(r['center_offset_pct'][1])>15]
