...
svg_rows=[r for r in rows if r.get('geometry_engine')=='svg2048']
if len(svg_rows) < 12:
    errors.append(f'Only {len(svg_rows)} true SVG reference glyphs; expected at least 12')

for r in rows:
    x,y=r['center_offset_pct']; c=r['coverage_pct']; k=r['contrast_estimate']
    is_svg=r.get('geometry_engine')=='svg2048'
    limit=10 if is_svg else 15
    if abs(x)>limit or abs(y)>limit:
        errors.append(f"{r['icon']}: optical center offset {x},{y}% (engine={r.get('geometry_engine')})")
    if c<2.5 or c>70:
...
print(f'Liquid27 QA: {len(rows)} icons; {len(svg_rows)} SVG2048 references; median coverage {median:.1f}%; mean contrast {statistics.mean(contrast):.3f}')
