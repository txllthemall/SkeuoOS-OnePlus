...
def _snell_flow(surface:dict,size:int,ior:float=1.49):
    """Two-interface visual lens flow. Direction comes from normals; magnitude
    comes from thickness/curvature. The shell deliberately has enough energy to
    change local line spacing at launcher-preview scale, not merely at 400% zoom."""
    ...
    # Normal-gradient term produces compression/stretch, not a rigid translation.
...
def _environment_reflection(arr,surface,dx,dy):
    ...
    # Broad environment response occupies a surface region, not a painted streak.
...
def composite_container(base:Image.Image):
    ...
    # Secondary internal interface samples a shorter, opposite optical path.
    ...
    spec=_specular(s)[...,None]; out=out*(1-spec*.30)+255.*spec*.30
    lx,ly=-.54,-.68; opposite=np.clip(s['nx']*(-lx)+s['ny']*(-ly),0,1)**2.6*np.clip(s['edge']*.45+s['fmid'],0,1); out*=1-opposite[...,None]*.10
    ...
    bright=np.clip((low-.70)/.25,0,1)*s['edge']*.070; dark=np.clip((.34-low)/.34,0,1)*s['fmid']*.065
    gain=1.+(1.-lc)*s['edge']*.10; rr=(rr-low[...,None])*gain[...,None]+low[...,None]; rr=np.clip(rr-bright[...,None]+dark[...,None],0,1)
...
def composite_glyph(base:Image.Image,glyph_mask:Image.Image):
    """Glyph is a second transparent lens. Identity comes from a second optical
    mapping and interfaces, not a white fill."""
    size=base.size[0]; s=glyph_surface(glyph_mask,size); arr=np.asarray(base.convert('RGB'),dtype=np.float32)
    eta=1./1.52; cosi=np.clip(s['nz'],0,1); k=np.clip(1.-eta*eta*(1.-cosi*cosi),0,1); scalar=eta+(eta*cosi-np.sqrt(k))
    ...
    alpha=np.clip(s['alpha'],0,1); edge=np.clip(s['edge'],0,1); fresnel=np.clip(s['fresnel']+edge*.24,0,.80)
    # Internal second surface makes checker/rails visibly change twice.
    inner=_bilinear_warp(arr,-dx*.22,-dy*.22).astype(np.float32); iw=(edge*.10)[...,None]
    glass=warped*(1-iw)+inner*iw
    env=_bilinear_warp(arr,-dx*.34,-dy*.34).astype(np.float32); env_w=(fresnel*.15)[...,None]; glass=glass*(1-env_w)+env*env_w
    lx,ly,lz=-.50,-.72,.48; lm=(lx*lx+ly*ly+lz*lz)**.5; lx,ly,lz=lx/lm,ly/lm,lz/lm
    ndotl=np.clip(s['nx']*lx+s['ny']*ly+s['nz']*lz,0,1); spec=(ndotl**34.)*np.clip(fresnel*1.25+edge*.18,0,.55)
    glass=glass*(1-spec[...,None]*.24)+255.*spec[...,None]*.24
    opposite=np.clip(-(s['nx']*lx+s['ny']*ly),0,1)**2.5*edge; glass*=1-opposite[...,None]*.12
    body_mix=np.clip(alpha*(.82+.12*edge),0,.92)[...,None]
